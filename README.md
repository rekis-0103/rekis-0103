# <img src="https://raw.githubusercontent.com/MartinHeinz/MartinHeinz/master/wave.gif" width="30px" height="30px" /> Hi there, I'm Rekishii Lucy!

<div align="center">
  <img src="https://readme-typing-svg.herokuapp.com/?lines=Software+Engineering+Student;Web+Development+Enthusiast;PHP+Developer;Always+Learning+New+Technologies!&font=Roboto&size=24&duration=3000&pause=1000&color=00D9FF&center=true&width=600" alt="Typing SVG" />
</div>

## 🚀 About Me

```php
<?php
class Developer {
    public $name = "Rekishii Lucy";
    public $role = "Software Engineering Student";
    public $language_spoken = ["id_ID", "en_US"];
    
    public function getSkills() {
        return [
            "backend" => ["PHP", "Laravel", "MySQL"],
            "frontend" => ["HTML5", "CSS3", "JavaScript"],
            "tools" => ["Git", "VS Code", "Laragon"],
            "learning" => ["Python", ".NET", "Modern Frameworks"]
        ];
    }
    
    public function getCurrentFocus() {
        return "Building amazing web applications with PHP 🚀";
    }
}
```

---

## 💼 What I Do

<table>
<tr>
<td width="50%">

### 🎯 Current Focus
- 🌟 Mastering **PHP** and **Laravel Framework**
- 🔧 Building scalable web applications
- 📚 Expanding knowledge in modern web technologies

</td>
<td width="50%">

### 🌱 Learning Journey
- 🐍 Exploring **Flutter** for mobile development
- ⚡ Diving into **Modern JavaScript** frameworks
- 🗄️ Advanced **Database** optimization

</td>
</tr>
</table>

---

## 🛠️ Tech Arsenal

<div align="center">

### Languages
<img src="https://skillicons.dev/icons?i=php,html,css,js,python,dotnet" />

### Frameworks & Libraries
<img src="https://skillicons.dev/icons?i=laravel,bootstrap" />

### Databases & Tools
<img src="https://skillicons.dev/icons?i=mysql,git,github,vscode" />

</div>

---

## 🎨 Featured Projects

<div align="center">
  
### 🍽️ Restaurant Website Management System
[![Website-Restoran](https://github-readme-stats.vercel.app/api/pin/?username=rekis-0103&repo=Website-Restoran&theme=tokyonight&hide_border=true)](https://github.com/rekis-0103/Website-Restoran)

*A comprehensive restaurant management platform built with PHP, featuring modern UI/UX and robust backend functionality*

**✨ Key Features:**
- 📱 Responsive design for all devices
- 🔐 Secure admin panel
- 📊 Real-time order management

</div>

---

## 📊 GitHub Analytics

<div align="center">
  <img width="49%" src="https://github-readme-stats.vercel.app/api?username=rekis-0103&show_icons=true&theme=tokyonight&hide_border=true&count_private=true" />
  <img width="49%" src="https://github-readme-streak-stats.herokuapp.com/?user=rekis-0103&theme=tokyonight&hide_border=true" />
</div>

<div align="center">
  <img width="60%" src="https://github-readme-stats.vercel.app/api/top-langs/?username=rekis-0103&theme=tokyonight&hide_border=true&layout=compact&langs_count=8" />
</div>

<div align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=rekis-0103&theme=tokyo-night&hide_border=true&area=true" width="100%"/>
</div>

---

## 🏆 GitHub Trophies

<div align="center">
  <img src="https://github-profile-trophy.vercel.app/?username=rekis-0103&theme=tokyonight&no-frame=true&column=7" />
</div>

---

## 🤝 Let's Connect!

<div align="center">

[![Instagram](https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white)](https://instagram.com/@ggorekis)
[![Instagram](https://img.shields.io/badge/Youtube-F38861?style=for-the-badge&logo=youtube&logoColor=white)](https://youtube.com/@ggorekis_gaming)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:rekishiilucy658@gmail.com)

</div>

---

<div align="center">
  
### 💫 Fun Facts About Me

🎮 **Gaming:** Love playing shooter & RPG games  
☕ **Coffee:** Fueled by coffee and curiosity  
📚 **Reading:** Always reading tech blogs and documentation, Also novels and mangas  
🎵 **Music:** Code + Music makes me more productive 

</div>

---

<div align="center">
  <img src="https://komarev.com/ghpvc/?username=rekis-0103&style=for-the-badge&color=blue" alt="Profile Views" />
  
  <h3>✨ Thanks for visiting my profile! ✨</h3>
  
  <img src="https://raw.githubusercontent.com/platane/platane/output/github-contribution-grid-snake-dark.svg" alt="Snake animation" />
  
</div>

---

<!--
**rekis-0103/rekis-0103** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

-->

<div align="center">
  <em>Happy Coding! 🚀</em>
</div>
